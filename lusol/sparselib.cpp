...
sparseMatrix *createMatrix(int dimLimit, int lenLimit, int initVectors)
{
  ...
  if (!(matrix = (sparseMatrix *) calloc((size_t)(1), sizeof(sparseMatrix))) && 1) { printf("calloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
  //CALLOC(matrix, 1);
...
void resizeMatrix(sparseMatrix *matrix, int newSize)
{
  ...
  if (!(matrix->list = (sparseVector **) realloc(matrix->list, sizeof(sparseVector*))) && newSize) { printf("realloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
...
sparseVector *createVector(int dimLimit, int initSize)
{
  ...
  if (!(newitem = (sparseVector *) calloc((size_t)(1), sizeof(sparseVector))) && 1) { printf("calloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
  //CALLOC(newitem, 1);
...
int resizeVector(sparseVector *sparse, int newSize)
{
  ...
  oldsize = sparse->size;
  if (!(sparse->value = (double *) realloc(sparse->value, sizeof(double))) && (newSize + 1)) { printf("realloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
  if (!(sparse->index = (int *) realloc(sparse->index, sizeof(int))) && (newSize + 1)) { printf("realloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
...
void dswapVector1(sparseVector *sparse, REAL *dense, int indexStart, int indexEnd)
{
  ...
  if(indexEnd <= 0)
    indexEnd = n;
  if (!(x = (double *) calloc((size_t)(MAX(indexEnd,n)+1), sizeof(double))) && (MAX(indexEnd,n)+1)) { printf("calloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
...
void dswapVector3(sparseVector *sparse1, sparseVector *sparse2, int indexStart, int indexEnd)
{
  ...
  if(indexStart <= firstIndex(sparse1) && indexStart <= firstIndex(sparse2) &&
	...
	if (!(dense1 = (double *) calloc((size_t)(indexEnd+1), sizeof(double))) && (indexEnd+1)) { printf("calloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
	if (!(dense2 = (double *) calloc((size_t)(indexEnd+1), sizeof(double))) && (indexEnd+1)) { printf("calloc of failed on line %d of file %s\n", __LINE__, __FILE__); }
