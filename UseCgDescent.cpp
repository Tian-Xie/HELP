...
// (A D^(-1) A^T) x_2 = (A D^(-1) b_1 - b_2)
double CG_DESCENT_ATX[MAX_COLS], CG_DESCENT_D[MAX_COLS];
double myvalue(double* x, INT n)
{
	// 1/2 x^T A x - b^T x

	// 1/2 x^T (A D^(-1) A^T) x - tmp_row^T x
	
	for (int i = 0; i < n_Col; i ++) // A^T x: (n_Col * 1)
		CG_DESCENT_ATX[i] = 0;
	char MKL_matdescra[6] = {0};
	MKL_matdescra[0] = 'G';
	MKL_matdescra[3] = 'C';
	mkl_dcsrmv(&CHAR_N, &n_Col, &n_Row, &DOUBLE_ONE, MKL_matdescra, csrValAt, csrColIndAt, csrRowPtrAt, csrRowPtrAt + 1, x, &DOUBLE_ONE, CG_DESCENT_ATX);
	double value = 0;
	for (int i = 0; i < n_Col; i ++)
		value += 0.5 * CG_DESCENT_ATX[i] * CG_DESCENT_ATX[i] / CG_DESCENT_D[i];
...
void mygrad(double* g, double* x, INT n)
{
	...
	// (A D^(-1) A^T) x - tmp_row
	for (int i = 0; i < n_Col; i ++)
		CG_DESCENT_ATX[i] /= CG_DESCENT_D[i];
	for (int i = 0; i < n_Row; i ++)
		g[i] = -tmp_row[i];
	mkl_dcsrmv(&CHAR_T, &n_Col, &n_Row, &DOUBLE_ONE, MKL_matdescra, csrValAt, csrColIndAt, csrRowPtrAt, csrRowPtrAt + 1, CG_DESCENT_ATX, &DOUBLE_ONE, g);
}
...
*/
	for (int i = 0; i < n_Col; i ++)
		CG_DESCENT_D[i] = d[i];
	printf("CG_DESCENT: %d\n", cg_descent(X2, n_Row, NULL, NULL, 1e-10, myvalue, mygrad, myvalgrad, NULL));
